...
class SystemState:
    ...
    def is_measurement_time_elapsed(self) -> bool:
        ...
        with self._lock:
            ...
            return elapsed >= self._state["defined_measurement_time"]
    ...
    def _start_runtime_timer(self) -> None:
        """Start timer to update runtime field"""
            ...
            while self._state["measurement_active"]:
                with self._lock:
                    if self._state["button_press_time"] is not None:
                        self._state["runtime"] = int(time.time() - self._state["button_press_time"])
...
def adc_sampling_thread() -> None:
    """
    Thread for ADC sampling during active measurements
    """
    logger.info("🔄 ADC sampling thread started")
    sample_count = 0
    ...
    while not shutdown_requested.is_set():
        try:
            if sampling_active.is_set():
                ...
                # Log the raw value we read
                logger.debug(f"ADC raw value: {raw_value}, voltage: {voltage:.4f}V")
                ...
                if sample_count % 100 == 0:
                    logger.info(f"ADC sample #{sample_count}: {raw_value} ({voltage:.3f}V, {field_strength:.2f}kV/cm)")
        ...
        try:
            ...
            if current_state == Q628State.POWER_UP:
                ...
                if system_state.is_measurement_time_elapsed():
                    logger.info(f"⏱️ Defined measurement time of {system_state.get('defined_measurement_time')}s reached")
...
def build_ttcp_data() -> bytes:
    ...
    try:
        ...
        runtime = state_dict["runtime"]  # Get runtime from state
        ...
        if not adc_data:
            adc_data.append(1000)  # Dummy value
        ...
        start_index = 0  # Normally from TCP_Data.StartMeasureIndex
