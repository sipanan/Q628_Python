...
class Config:
    ...
    FIXED_INITIALIZATION_TIME = 3 # Fixed initialization time for ADC/DAC
        ...
        if current_state == Q628State.IDLE and not state_dict["measurement_active"]:
            ...
            for i in range(50):
                if i < data_count:
                    ...
                    if raw_value & 0x200000:  # Check if bit 21 (sign bit) is set
                        delphi_value = raw_value | 0xFFC00000
                    else:
                        delphi_value = raw_value
                else:
                    delphi_value = 0
            ...
            logger.debug(f"📤 IDLE state: Sending {data_count} latest samples to maintain display")
            ...
            for i in range(50):
                if i < data_count:
                    ...
                    # Sign-extend 22-bit to 32-bit for Delphi compatibility
                    if value & 0x200000:  # Check if bit 21 (sign bit) is set
                        delphi_value = value | 0xFFC00000
                    else:
                        delphi_value = value
